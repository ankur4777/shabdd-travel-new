...
class DestinationController extends Controller
{
    ...
    private function normalizeAdminPackageForDestination(Package $package, Destination $destination): array
    {
        ...
        return [
            ...
            'gallery_images' => collect($package->detail_gallery ?? [])
                ->map(fn($image) => is_array($image) ? ($image['image'] ?? $image['path'] ?? $image['url'] ?? null) : $image)
            ...
            'faqs' => $this->normalizeFaqItems($package->faqs ?? []),
    ...
    private function buildPackagePageData(
        ...
        $mainImage = collect([
            ...
            $destination->image_url ? $this->mediaUrl($destination->image_url) : null,
        ])
            ->map(fn($image) => trim((string) $image))
            ->first(fn(string $image) => $image !== '') ?? $this->mediaUrl(null);
        ...
        $faqs = $this->normalizeFaqItems($selectedPackage['faqs'] ?? []);
        ...
        if (empty($faqs)) {
            $faqs = $this->normalizeFaqItems(!empty($destination->faqs) ? $destination->faqs : ($destinationProfile['faqs'] ?? []));
        }
    ...
    private function normalizeFaqItems($items): array
    {
        if (is_string($items)) {
            $decodedItems = json_decode($items, true);
            $items = json_last_error() === JSON_ERROR_NONE ? $decodedItems : [];
        }
